...
def compress_image(img, P, Q, filtre, n=8):
    nrows, ncols, _ = img.shape
...
def calculate_frobenius_norm(image_start, image_end):
    norme = np.linalg.norm((image_start - image_end), ord=2)
    return norme / np.linalg.norm(image_start)

def calculate_image_size(img):
    """
    Calcule la taille mémoire de l'image en octets, kilooctets et mégaoctets.
    """
    image_size_bytes = img.size * img.itemsize
    image_size_kb = image_size_bytes / 1024
    image_size_mb = image_size_kb / 1024
    return image_size_bytes, image_size_kb, image_size_mb

def display_images(original, decompressed_images, compression_rates, q_labels):
    # Affichage des images décompressées avec l'image originale
...
def main():
    ...
    les_gros_q = [Q_faible, Q_classique, Q_aggressif, Q_extreme, Q_max]
    q_labels = ["faible", "classique", "aggressif", "extrême", "max"]

    decompressed_images = []
    compression_rates = []

    for Q in les_gros_q:
        compressed = compress_image(img, P, Q, filtre)
